...
# Make file
mkdir -p ${DMRTYPE}/overlap${FEATUREID}/
...
bedtools intersect \
		  ...
		  > ${DMRTYPE}/overlap${FEATUREID}/${DMRTYPE}_${FEATUREID}_all_overlaps.with_coords.bed
# Drop coords for B, retain only ID
awk '{print $1"\t"$2"\t"$3"\t"$4";"$8"\t"$9}' \
		  ${DMRTYPE}/overlap${FEATUREID}/${DMRTYPE}_${FEATUREID}_all_overlaps.with_coords.bed \
		  > ${DMRTYPE}/overlap${FEATUREID}/${DMRTYPE}_${FEATUREID}_all_overlaps.bed

# Keep only unique DMRs that overlap
echo "#### Keeping only unique DMRs"
grep \
    -w -f \
    <( cat ${DMRTYPE}/overlap${FEATUREID}/${DMRTYPE}_${FEATUREID}_all_overlaps.bed | awk '{print $4 }' | sed 's/[;].*//g' ) \
    ${DMRTYPE}/$AFILENAME.withids.pFilt${PVALTHR}.bed |\
    uniq \
    > ${DMRTYPE}/overlap${FEATUREID}/unique_${DMRTYPE}_${FEATUREID}_overlaps.bed

# Get details on ATAC-overlapping DMRs
grep \
    -w -f \
    <( cat ${DMRTYPE}/overlap${FEATUREID}/unique_${DMRTYPE}_${FEATUREID}_overlaps.bed | awk '{print $4}' ) \
    $AFILE \
    > ${DMRTYPE}/overlap${FEATUREID}/$AFILENAME.${FEATUREID}overlap.tsv
cat \
   <( head -n 1 $AFILE ) \
   ${DMRTYPE}/overlap${FEATUREID}/$AFILENAME.${FEATUREID}overlap.tsv \
   > /tmp/k && mv /tmp/k \
...
grep \
    -w -f \
    <( cat ${DMRTYPE}/overlap${FEATUREID}/${DMRTYPE}_${FEATUREID}_all_overlaps.with_coords.bed | awk '{print $8}' ) \
    $BFILENAME.withids.bed \
    > ${DMRTYPE}/overlap${FEATUREID}/$BFILENAME.${FEATUREID}overlap.bed
