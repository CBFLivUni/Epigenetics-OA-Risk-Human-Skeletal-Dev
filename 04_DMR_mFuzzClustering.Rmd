...
# Load in m-lvaues
mvals <- read.delim(paste0(res_dir, "methylation_matrix.mvals.tsv"), sep="\t", header=TRUE, row.names=1)
...
# Keep just sig CpGs
sigCpG_stage <- sigCpG_stage[sigCpG_stage$adj.P.Val < 0.001,]
sigCpG_sex <- sigCpG_sex[sigCpG_sex$adj.P.Val < 0.05,]
...
StageDmr_df <- StageDmr_df[,
  c("DMR_ID","SITE","chr","start","end","SYMBOL","DISTANCE","B","S","estimate","se","z","p.value","p.adjust","CONTRAST","MAXGAP","CpG","pos","CLUSTER","logFC","P.Value","adj.P.Val","n","nSigCpg","nNonSigCpg","pSigCpg")]
colnames(StageDmr_df)[c(13,14,20,21,22)] <- c("DMR_PVAL","DMR_PADJ","CPG_ESTIMATE","CPG_PVAL","CPG_PADJ")
SexDmr_df <- SexDmr_df[,
  c("DMR_ID","SITE","chr","start","end","SYMBOL","DISTANCE","B","S","estimate","se","z","p.value","p.adjust","CONTRAST","MAXGAP","CpG","pos","CLUSTER","logFC","P.Value","adj.P.Val","n","nSigCpg","nNonSigCpg","pSigCpg")]
colnames(SexDmr_df)[c(13,14,20,21,22)] <- c("DMR_PVAL","DMR_PADJ","CPG_ESTIMATE","CPG_PVAL","CPG_PADJ")
...
# Read in
CpGTrt_df <- read.table(paste0(de_dir, "differential_methylation.limma_results.continuous_covariates.treat_log2FC",trt_log2fc_thr,"_thr.padj_thr",0.001,".tsv"), sep="\t", header=TRUE, quote = "")
...
for (direction in c("all")) {
  ...
  if (direction == "up") { 
    ...
    up_goCpGStage <- RunMethylEnrichment(stageCpG, groupCol="CLUSTER", cpg_df=backSet[,c("Name","CONTRAST")], groupIsBackGroundSubset=FALSE,
                                               ...
                                               pval_col="adj.P.Val", dir=goclust_dir, CpGCol="Name", extra=paste0(".",direction,"CpGStage"))
